...
task :generate_insights => :environment do
  user = User.where("last_insight_at < ? OR last_insight_at IS NULL", 24.hours.ago).order("last_insight_at NULLS FIRST")&.first
  ...
  if user
    ...
    if locations && user.places
      ...
      locations.each_with_index do |location, idx|
        ...
        elsif (current_place.nil? && !previous_place.nil?) || ((idx + 1) == locations.count && !current_place.nil?)
